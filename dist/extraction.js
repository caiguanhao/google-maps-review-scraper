 ...
 */
async function fetchPlaceDetails(previewUrl) {
    let name = null;
    let address = null;
 ...
 */
export default async function fetchPlaceData(url, placeId) {
    try {
        ...
        if (!token)
            throw new Error("Could not find session token (kEI) in source.");
